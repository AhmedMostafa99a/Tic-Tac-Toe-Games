#ifndef _MINMAXPLAYER5X5_H
#define _MINMAXPLAYER5X5_H

#include "BoardGame_Classes.h"

template <typename T>
class X_O_MinMax5x5_Player : public Player<T> {
public:
    X_O_MinMax5x5_Player(string Name,T symbol);
...
template <typename T>
X_O_MinMax5x5_Player<T>::X_O_MinMax5x5_Player(string Name,T symbol) : Player<T>(symbol) {
    this->name = Name;
}

// Method to get the best move for the player
template <typename T>
void X_O_MinMax5x5_Player<T>::getmove(int& x, int& y) {
...
template <typename T>
int X_O_MinMax5x5_Player<T>::calculateMinMax(T s, bool isMaximizing) {
...
template <typename T>
std::pair<int, int> X_O_MinMax5x5_Player<T>::getBestMove() {
